...
async function getScansPerDate() {
  ...
  const { rows } = await executeSql(`SELECT time_of_scan FROM scans WHERE time_of_scan > ?`, [Date.now() - 5 * 24 * 60 * 60 * 1000]);
  console.log(rows);

  // return in format {labels: [dd/mm, dd/mm, ...], datasets: [{data: [nbrOfScans, nbrOfScans, ...], color: (opacity = 1) => `rgba(99, 66, 232, ${opacity})`}]}
  let labels = [];
  let datasets = [{data: [], color: (opacity = 1) => `rgba(99, 66, 232, ${opacity})`}];
  ...
  for (let i = 4; i >= 0; i--) {
    ...
    for (let row of rows) {
      ...
      if (scanDate.getDate() === date.getDate() && scanDate.getMonth() === date.getMonth()) {
        count++;
      }
    }
...
async function getPieData() {
  const categories = [
    ...
    { name: 'M/G/P', types: ['metal', 'glass', 'plastic'] },
...
export { deleteDatabase, getLevel, getNbrOfScans, getPieData, getStreak, insertNewScan, getScansPerDate };
