...
async function getStreak() {
  ...
  let scanDates = rows.map(
    (r) => new Date(r.time_of_scan)
  );
  ...
  for (let i = 0; i < scanDates.length; i++) {
    ...
    if (scanDates[i].toLocaleDateString() === compareDate.toLocaleDateString()) {
      // console.log('date already seen');
      continue;
    }

    if (scanDates[i].toLocaleDateString() === expectedDate.toLocaleDateString()) {
      compareDate = scanDates[i];
      streak++;
    } else {
      break;
